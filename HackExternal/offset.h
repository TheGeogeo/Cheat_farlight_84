...
#define GameVars BloodHunt::Variables::Get()
	...
	public:
		...
		DWORD offset_component_to_world = 0x14c; // bComponentToWorldUpdated  0x14c 0x1c0 0x14c 0x4701c0 truct FTransform K2_GetComponentToWorld(); // Function Engine.SceneComponent.K2_GetComponentToWorld      0F 28 81 ? ? ? ? 48 8B C2 0F 28 89 ? ? ? ? 0F 29 02
		...
		DWORD offset_WeaponTextType = 0x6d8; //FString  WeaponTextType;  // 0x0868   (0x0010)
		DWORD offset_Hp = 0x5d8; // //ASolarPlayerState  float CurrentHealth; // 0x580(0x04)
		DWORD offset_HpMax = 0x5dc; // ASolarPlayerState float MaxHealth; // 0x584(0x04)
		DWORD offset_ShieldInfo = 0x5e0;
		DWORD offset_ShieldLevel = 0x0; // offset_ShieldInfo + 0x
		DWORD offset_Shield = 0x4; // offset_ShieldInfo + 0x
		DWORD offset_ShieldMax = 0x10f4;
		DWORD offset_player_name = 0x310; // APlayerState 	PlayerNamePrivate 	0x300 	FString
		DWORD offset_ASolarTeamInfotTeam = 0xb60; // SolarTeamInfo* Team
		DWORD offset_ASolarTeamInfotTeamID = 0x0240; // SolarTeamInfo->TeamID
		DWORD offset_KillNum = 0x7a8; // int KillNum
		DWORD offset_bot = 0x232;  // PlayerState bool bIsABot
		DWORD offset_ItemData = 0x270; // Class: SolarItemActor : SolarItemData ItemData
